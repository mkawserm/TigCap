...
void PieceBoard::debugMoveInMask(int a,int b)
{
    bool flag = false;
    if( P[a]->isBlack() ) flag = true , P[b]->setBLACK();
    else if( P[a]->isRed() ) P[b]->setRED();
    P[a]->setBoardVertex();
    if( flag )
    {
        maskBlack = _flipAt( maskBlack , idInM[ a ] );
        maskBlack = _flipAt( maskBlack , idInM[ b ] );
    }
    else
    {
        maskRed = _flipAt( maskRed , idInM[ a ] );
        maskRed = _flipAt( maskRed , idInM[ b ] );
    }
